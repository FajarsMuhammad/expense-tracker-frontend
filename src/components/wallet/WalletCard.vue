  ...
  <AppCard class="hover:shadow-soft-xl transition-all duration-300 cursor-pointer group" @click="$emit('view', wallet)">
    <div class="flex items-center justify-between gap-2 md:gap-3 mb-3 md:mb-4">
      <div class="flex-1 min-w-0">
        <p class="text-[10px] md:text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-0.5 md:mb-1">{{ $t('wallets.wallet') }}</p>
    ...
    <div class="mb-3 md:mb-4">
      <p class="text-[10px] md:text-xs text-neutral-500 dark:text-neutral-400 mb-1">{{ $t('wallets.currentBalance') }}</p>
    ...
    <div class="flex gap-1.5 md:gap-2 pt-2 md:pt-3 border-t border-neutral-200 dark:border-dark-border">
      <AppButton size="sm" variant="secondary" @click.stop="$emit('edit', wallet)" class="flex-1 rounded-lg bg-neutral-100 px-2 md:px-3 py-1.5 md:py-2 text-[10px] md:text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-700">
